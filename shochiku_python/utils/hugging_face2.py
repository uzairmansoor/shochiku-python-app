...
class Hugging:
    ...
    def text_to_image(self, prompt, temp_file_path, loramodel, seed , width=512, height=512):
        ...
        print("SEED: ", seed)
        print("prompt: ", prompt)
        print("loramodel: ", loramodel)
        #print("image: ", image)
        #print("strength: ", strength)
        seed = np.random.randint(100000000) + random.randint(0,os.sys.maxsize)
        ...
        print("image: ", image, "file_path: ", temp_file_path)
    ...
    def image_to_image(self, prompt, temp_file_path,loramodel, image, seed=42, strength=0.8):
        ...
        print("prompt: ", prompt)
        print("loramodel: ", loramodel)
        print("image: ", image, "temp_file_path: ", temp_file_path)
        print("strength: ", strength)
        seed = 42
        generator = torch.Generator(device=self.device).manual_seed(int(seed))
        print('AR: ', image.size)
        ...
        image = self.pipe( prompt=prompt, image=image, strength=strength, num_inference_steps=1, guidance_scale=0, generator=generator).images[0]
...
def get_image_from_text1(event):
    data = event['body']
    prompt = data['prompt']
